...
import useInterval from "@/hooks/brh/useInterval";
import useHorizontalSwipe from "@/hooks/brh/useHorizontalSwipe";
...
export const SectionHero: FC<SectionHeroProps> = ({ className = "" }) => {
          ...
          return (
            ...
            >
              <div className={`relative w-20 h-1 shadow-sm rounded-md bg-white`}>
                {isActive && <div className={`nc-SectionHero2Item__dot absolute inset-0 bg-slate-900 rounded-md ${isActive ? " " : " "}`}></div>}
              </div>
            </div>
          );
        })}
  ...
  const renderItem = (index: number) => {
    ...
    return (
      <div className={`nc-SectionHero2Item nc-SectionHero2Item--animation flex flex-col-reverse lg:flex-col relative overflow-hidden ${className}`} key={index}>
            ...
            <div className="container relative">
              <div className="relative z-[1] w-full max-w-3xl space-y-8 sm:space-y-14 nc-SectionHero2Item__left">
                <div className="space-y-5 sm:space-y-6">
                  <span className="nc-SectionHero2Item__subheading block text-base md:text-xl text-slate-700 font-medium">{item.subHeading}</span>
                  <h2
                ...
                <ButtonPrimary className="nc-SectionHero2Item__button dark:bg-slate-900" sizeClass="py-3 px-6 sm:py-5 sm:px-9" href={item.btnLink}>
